...
# CONFIG
# ---------------------------
GROUP_ID = -1003570284022  # your Telegram group ID
...
# ---------------------------
# FLASK WEB ENDPOINT (for Render / Uptime)
# ---------------------------
app = Flask(__name__)
...
def is_authorized(user_id: int) -> bool:
    """
    Returns True if the user is in the group (member/admin/creator), False otherwise.
    """
    try:
...
def deny_access(chat_id):
    bot.send_message(
        ...
        "Please join the official group to have access.",
        parse_mode="HTML"
    )
...
def authorized_only(func):
    """
    Decorator to check if a user is authorized before running a handler.
    """
...
def main_menu():
    ...
    markup.add(
        types.KeyboardButton("🦴 Upper Limb Osteology"),
        types.KeyboardButton("🩺 Fascia / Vessels / Cutaneous Nerves"),
        types.KeyboardButton("💪 Muscles of the Upper Limb"),
        types.KeyboardButton("🤝 Joints of the Upper Limb"),
        types.KeyboardButton("📍 Major Landmarks"),
        types.KeyboardButton("🧠 Brachial Plexus"),
        types.KeyboardButton("🚑 Clinical Correlations"),
        types.KeyboardButton("📚 About Bot")
    )
    return markup
...
def osteology_menu():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(
        types.KeyboardButton("Clavicle"),
        types.KeyboardButton("Scapula"),
        types.KeyboardButton("Humerus"),
        types.KeyboardButton("Radius and Ulna"),
        types.KeyboardButton("Hand Bones"),
...
def fascia_vessels_menu():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(
        types.KeyboardButton("Fascia"),
        types.KeyboardButton("Vessels"),
        types.KeyboardButton("Cutaneous Nerves"),
        types.KeyboardButton("⬅ Back")
    )
    return markup
...
def muscles_menu():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(
        types.KeyboardButton("Muscles of the Upper Arm"),
        types.KeyboardButton("Muscles of the Forearm"),
        types.KeyboardButton("Small Muscles of the Hand"),
        types.KeyboardButton("The Rotator Cuff (SITS)"),
        types.KeyboardButton("Vertebral Column Connections"),
        types.KeyboardButton("Thoracic Wall Connections"),
        types.KeyboardButton("Scapula to Humerus Connections"),
        types.KeyboardButton("⬅ Back")
    )
    return markup
...
def rotator_cuff_menu():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(
        types.KeyboardButton("Rotator Cuff Overview"),
        types.KeyboardButton("Supraspinatus"),
        types.KeyboardButton("Infraspinatus"),
        types.KeyboardButton("Teres Minor"),
        types.KeyboardButton("Subscapularis"),
        types.KeyboardButton("⬅ Back to Muscles")
    )
    return markup
...
def joints_menu():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(
        types.KeyboardButton("Shoulder Joint"),
        types.KeyboardButton("Elbow Joint"),
        types.KeyboardButton("Radioulnar Joints"),
        types.KeyboardButton("Wrist Joint"),
        types.KeyboardButton("Hand Joints"),
        types.KeyboardButton("⬅ Back")
    )
    return markup
...
def landmarks_menu():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(
        types.KeyboardButton("The Axilla"),
        types.KeyboardButton("Cubital Fossa"),
        types.KeyboardButton("The Carpal Tunnel"),
        types.KeyboardButton("The Guyon's Canal"),
        types.KeyboardButton("The Anatomical Snuffbox"),
        types.KeyboardButton("⬅ Back")
    )
    return markup
...
def brachial_plexus_menu():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(
        types.KeyboardButton("Formation"),
        types.KeyboardButton("Roots (C5-T1)"),
        types.KeyboardButton("Trunks"),
        types.KeyboardButton("Divisions"),
        types.KeyboardButton("Cords"),
        types.KeyboardButton("Supraclavicular Branches"),
        types.KeyboardButton("Infraclavicular Branches"),
        types.KeyboardButton("Major Terminal Nerves"),
        types.KeyboardButton("⬅ Back")
    )
    return markup
...
def clinical_menu():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(
        types.KeyboardButton("Fractures"),
        types.KeyboardButton("Dislocations and Joint Injuries"),
        types.KeyboardButton("Nerve Injuries"),
        types.KeyboardButton("Vascular Injuries"),
        types.KeyboardButton("Muscle and Tendon Injuries"),
        types.KeyboardButton("Special Conditions"),
        types.KeyboardButton("Tests"),
        types.KeyboardButton("⬅ Back")
    )
    return markup
...
def deny_access(chat_id):
    bot.send_message(
        ...
        "⛔ <b>You are not authorized to use this bot.</b>\n\n"
        "Please join the official group to have access.\n"
        "Contact admin @JoshuaJB558",
        parse_mode="HTML"
    )

# ---------------------------
# HELPER: SEND AND TRACK MESSAGE
# ---------------------------
def send_tracked_message(user_id, text, reply_markup=None):
    msg = bot.send_message(user_id, text, reply_markup=reply_markup, parse_mode="Markdown")
...
def send_welcome(message):
    ...
    notice_text = (
        "📢 <b>Notice to Users</b>\n\n"
        "This bot is for personal study and revision only.\n\n"
        "Screenshotting, forwarding, or compiling content into PDFs is at your own discretion.\n\n"
        "• The content here is regularly updated, corrected, and expanded.\n"
        "• Saved or shared copies may become outdated or incomplete over time.\n\n"
        "If you think you're \"clever\" enough to archive it - good luck keeping up."
    )
    send_tracked_message(message.chat.id, notice_text)
    ...
    send_tracked_message(message.chat.id, welcome_text, reply_markup=main_menu())
...
def handle_menus(message):
    ...
    # ---------------------------
    menus = {
        "🦴 Upper Limb Osteology": osteology_menu(),
        "🩺 Fascia / Vessels / Cutaneous Nerves": fascia_vessels_menu(),
        "💪 Muscles of the Upper Limb": muscles_menu(),
        "The Rotator Cuff (SITS)": rotator_cuff_menu(),
        "🤝 Joints of the Upper Limb": joints_menu(),
        "📍 Major Landmarks": landmarks_menu(),
        "🧠 Brachial Plexus": brachial_plexus_menu(),
        "🚑 Clinical Correlations": clinical_menu(),
        "⬅ Back": main_menu(),
        "⬅ Back to Muscles": muscles_menu(),
        "📚 About Bot": None
    }
    ...
    if text in menus:
        if text == "📚 About Bot":
            ...
            send_tracked_message(message.chat.id, about_text)
        else:
            send_tracked_message(message.chat.id, f"Select an option:", reply_markup=menus[text])
        return
    ...
    if text in content_map:
        send_tracked_message(message.chat.id, content_map[text])

# ---------------------------
# DELETE MESSAGES WHEN USER LEAVES
# ---------------------------

@bot.my_chat_member_handler()
def handle_user_left(my_chat_member):
